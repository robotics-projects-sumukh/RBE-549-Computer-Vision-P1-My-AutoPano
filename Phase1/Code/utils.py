...
def create_corners_overlay(image, corner_coords, color=(0, 0, 255)):
    ...
    for row, col in corner_coords:  # Swapping to (col, row) format
        cv2.circle(overlay, (int(col), int(row)), 4, color, -1)
...
def ANMS(Cimg, NumFeatures, filter_size=5):
    """
    Adaptive Non-Maximal Suppression (optimized version)
    """
    # Find local maxima
    lm = maximum_filter(Cimg, size=filter_size) == Cimg
...
def get_panorama(img1, img2, blockSize=2, ksize=3, k=0.03, nBest=1000, Set=1, SetType="Train", i=1, blend=False, panorama_type="left"):
    flag = False
    """
    Corner Detection
    ...
    drawImage(corner_overlay1, "corner_overlay", Set, SetType, "Corners", i-1)   
    drawImage(corner_overlay2, "corner_overlay", Set, SetType, "Corners", i)
    """
    ...
    anms_points1 = ANMS(corner_score1, nBest, filter_size=filter_size)
    anms_points2 = ANMS(corner_score2, nBest, filter_size=filter_size)
    # print("ANMS completed.")
    
    anms_overlay1 = create_corners_overlay(img1, anms_points1, color=(0, 255, 0))
    anms_overlay2 = create_corners_overlay(img2, anms_points2, color=(0, 255, 0))
    drawImage(anms_overlay1, "anms_overlay", Set, SetType, "ANMS", i-1)
    drawImage(anms_overlay2, "anms_overlay", Set, SetType, "ANMS", i)    
    """
    Feature Descriptors
    ...
    drawImage(fd_img_1, "fd", Set, SetType, "FD", i-1)
    fd_img_2 = img2.copy()
    for cord in coordinates_2:
        x2, y2 = cord
        cv2.circle(fd_img_2, (y2,x2), 2, (255,0,0), -1)
    drawImage(fd_img_2, "fd", Set, SetType, "FD", i)
    ...
    matches = match_features(feature_descriptor1, feature_descriptor2, ratio_threshold=ratio_threshold)
    ...
    drawImage(matching_image, "matching", Set, SetType, "Features", i-1)
    ...
    # Plot the inliers where alternate points are from img1 and img2
    inlier_image = plot_ransac_inliers(img1, img2, keypoints_1, keypoints_2, inliers)
    drawImage(inlier_image, "ransac", Set, SetType, "ransacFeatures", i-1)    
    print("Homographies computed, H: ", H)
    """
    ...
    # Warp Images
    if blend:
        panorama = get_blended_warped_image(img2, img1, H)
    else:
        panorama = get_warped_image(img2, img1, H)
    drawImage(panorama, "mypano", Set, SetType, "Panorama", panorama_type)
