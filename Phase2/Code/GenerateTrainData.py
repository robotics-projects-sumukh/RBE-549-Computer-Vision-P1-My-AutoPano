...
def main():
    homography_train = dict()
    corners = dict()
    patch_size = 128
    perturbation_range = 32
    train_img_path = "../Data/Train/"
    labels_path = "TxtFiles/"
    patches_path = "../Data/GeneratedTrainData/"
    ...
    for i in tqdm(range(len(os.listdir(train_img_path)))):  #
        ...
        for j in range(2):
            ...
            homography_train[str(i + 1) + "_" + str(j + 1)] = H4pt
            corners[str(i + 1) + "_" + str(j + 1)] = corner_coords

    homography_train = np.array(homography_train)
    corners = np.array(corners)

    saveLabels(homography_train, corners, labels_path, type="train")

    end_time = time.time()
    print("Total Time: ", end_time-start_time)
    np_test = np.load(
        labels_path + "homography_train_labels.npy", allow_pickle=True
    )
